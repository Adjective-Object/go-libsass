...
if [ "${INSTALL_ZSH}" = "true" ]; then
    ...
    if [ ! -d "${oh_my_install_dir}" ] && [ "${INSTALL_OH_MYS}" = "true" ]; then
        ...
        git clone https://github.com/zsh-users/zsh-autosuggestions ${oh_my_install_dir}/plugins/zsh-autosuggestions
        sed -i -e 's/^plugins=\((...)\)/plugins=(git zsh-autosuggestions)/g' ${user_rc_file}
